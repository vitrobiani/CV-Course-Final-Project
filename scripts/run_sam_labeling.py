#!/usr/bin/env python3
"""
Run SAM 3 labeling on the Flickr dataset.

Usage:
    python scripts/run_sam_labeling.py [--max-images N] [--no-resume] [--batch-size N]
"""
import argparse
import logging
import sys
...
from src.config import FLICKR_IMAGES_DIR, SAM_LABELS_DIR, COMPARISONS_DIR, DEFAULT_SAM_CONFIG
from src.labeling.sam_labeler import SAMLabeler, create_sam_labeler
from src.validation.tta_validator import TTAValidator
from src.utils.dataset_utils import BoundingBox


def main():
    parser = argparse.ArgumentParser(description="Run SAM 3 labeling on Flickr dataset")
    ...
    # Setup
    output_dir = Path(args.output_dir) if args.output_dir else SAM_LABELS_DIR
    ...
    logger.info(f"Starting SAM 3 labeling")
    ...
    try:
        ...
        logger.info("=" * 50)
        logger.info("SAM 3 Labeling Complete")
        logger.info("=" * 50)
        ...
        if args.validate:
            ...
            tta_output = COMPARISONS_DIR / "tta_validation_sam.json"
